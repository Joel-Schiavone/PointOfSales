<?php
  ...
  //Inicio Trae todos los usuarios por usuario y clave
	...
	class mov_cajaE
	{
                        ...
                        {
                              $sql_mov_caja = 'SELECT pre_cant, mov_sal, pre_iva FROM mov_caja, precios WHERE mov_caja.ID_pre=precios.ID_pre AND ID_mov='.$ID_mov.' ' ; 
		...
		//Inicio Funcion trae los netos de los movimientos correspondientes a una venta y una caja
			  ...
			  function get_mov_cajaByIdVen($ID_ven)
			    {
			      $sql_mov_caja = 'SELECT * FROM mov_caja, precios WHERE precios.ID_pre=mov_caja.ID_pre AND ID_ven='.$ID_ven.' ' ; 
			      $result_mov_caja =mysql_query($sql_mov_caja);
			      return $result_mov_caja;
			    }
...
//Inicio Trae la ultima caja por usuario
	...
	class comprobantesE
	{
          ...
          //Fin Funciones para Mostrar Datos por ID
                        ...
                        {
                        	$sql_cabecera_comprobantes = 'SELECT *  FROM cabecera_comprobantes, tipo_comprobantes WHERE cabecera_comprobantes.ID_tce=tipo_comprobantes.ID_tce AND ID_fce=2 and (tce_movcaja=1 OR tipo_comprobantes.ID_tce=1 ) AND cte_fec BETWEEN "'.$fecDesde.'" AND "'.$fecHasta.'" AND tipo_comprobantes.ID_tce="'.$ID_tceB.'" ORDER BY cte_fec DESC';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }
                        ...
                        {
                        	$sql_cabecera_comprobantes = 'SELECT tce_detalleArticulos, ID_cte, cte_monto, cte_neto, cte_retencion, cte_metrica_descuento FROM cabecera_comprobantes, tipo_comprobantes WHERE cabecera_comprobantes.ID_tce=tipo_comprobantes.ID_tce AND ID_fce=2 and (tce_movcaja=1 OR tipo_comprobantes.ID_tce=1) AND cte_fec BETWEEN "'.$fecDesde.'" AND "'.$fecHasta.'" AND tipo_comprobantes.ID_tce="'.$ID_tceB.'" ORDER BY cte_fec DESC';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }
                        ...
                        {
                        	$sql_cabecera_comprobantes = 'SELECT * FROM tipo_comprobantes WHERE ID_fce=2 and tce_movcaja=1 or ID_tce=1';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }
						...
						function get_cabecera_comprobantes_listado_compras($fecDesde, $fecHasta)
                        {
                        	$sql_cabecera_comprobantes = 'SELECT *  FROM cabecera_comprobantes, tipo_comprobantes WHERE cabecera_comprobantes.ID_tce=tipo_comprobantes.ID_tce AND ID_fce=1 and tce_movcaja=1 AND cte_fec BETWEEN "'.$fecDesde.'" AND "'.$fecHasta.'" ORDER BY cte_fec DESC';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }

                        function get_cabecera_comprobantes_listado_compras_totales($fecDesde, $fecHasta)
                         {
                        	$sql_cabecera_comprobantes = 'SELECT tce_detalleArticulos, ID_cte, cte_monto, cte_neto, cte_retencion, cte_metrica_descuento FROM cabecera_comprobantes, tipo_comprobantes WHERE cabecera_comprobantes.ID_tce=tipo_comprobantes.ID_tce AND ID_fce=1 and tce_movcaja=1 AND cte_fec  BETWEEN "'.$fecDesde.'" AND "'.$fecHasta.'" AND cte_metrica_descuento ORDER BY cte_fec DESC';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }


                        function get_cabecera_comprobantes_listado_compras_ID_tceB($ID_tceB, $fecDesde, $fecHasta)
                        {
                        	$sql_cabecera_comprobantes = 'SELECT *  FROM cabecera_comprobantes, tipo_comprobantes WHERE cabecera_comprobantes.ID_tce=tipo_comprobantes.ID_tce AND ID_fce=1 and tce_movcaja=1 AND cte_fec BETWEEN "'.$fecDesde.'" AND "'.$fecHasta.'" AND ID_tceB="'.$ID_tceB.'" ORDER BY cte_fec DESC';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }

                        function get_cabecera_comprobantes_listado_compras_ID_tceB_totales($ID_tceB, $fecDesde, $fecHasta)
                        {
                        	$sql_cabecera_comprobantes = 'SELECT tce_detalleArticulos, ID_cte, cte_monto, cte_neto, cte_retencion, cte_metrica_descuento FROM cabecera_comprobantes, tipo_comprobantes WHERE cabecera_comprobantes.ID_tce=tipo_comprobantes.ID_tce AND ID_fce=1 and tce_movcaja=1 AND cte_fec BETWEEN "'.$fecDesde.'" AND "'.$fecHasta.'" AND ID_tceB="'.$ID_tceB.'" ORDER BY cte_fec DESC';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }

                        function get_cabecera_comprobantes_listado_compras_select()
                        {
                        	$sql_cabecera_comprobantes = 'SELECT *  FROM tipo_comprobantes WHERE ID_fce=1 and tce_movcaja=1';
                        	  $result_cabecera_comprobantes =mysql_query($sql_cabecera_comprobantes);
                              return $result_cabecera_comprobantes;
                        }
